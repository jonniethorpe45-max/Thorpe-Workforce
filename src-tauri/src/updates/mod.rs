...
pub async fn check_for_updates(current_version: &str, api_url: Option<&str>) -> Result<UpdateInfo, String> {
    let release = match fetch_latest_release(api_url).await {
        ...
        Err(err) => {
            return Ok(UpdateInfo {
                ...
                release_notes: "Could not verify the latest release. Check your connection and try again.".to_string(),
    ...
    if release.draft.unwrap_or(false) || release.prerelease.unwrap_or(false) {
        return Ok(UpdateInfo {
            ...
            latest_version: current_version.to_string(),
            update_available: false,
            release_notes: "No public release is available yet.".to_string(),
            download_url: release.html_url,
            check_error: Some("Latest release is not publicly available yet.".to_string()),
        });
    }
